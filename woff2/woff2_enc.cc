...
bool Compress(const uint8_t* data, const size_t len, uint8_t* result,
  ...
  *result_len = (uint32_t)compressed_len;
  return true;
}
...
size_t ComputeUncompressedLength(const FontCollection& font_collection) {
  ...
  size_t size = CollectionHeaderSize(font_collection.header_version,
    (uint32_t)font_collection.fonts.size());
  for (const auto& font : font_collection.fonts) {
...
bool ConvertTTFToWOFF2(const uint8_t *data, size_t length,
  ...
  size_t compression_buffer_size = CompressedBufferSize((uint32_t)total_transform_length);
  std::vector<uint8_t> compression_buf(compression_buffer_size);
  uint32_t total_compressed_length = (uint32_t)compression_buffer_size;
  ...
  uint32_t compressed_metadata_buf_length =
    CompressedBufferSize((uint32_t)params.extended_metadata.length());
  ...
  *result_length = woff2_length;
  ...
  StoreU32((uint32_t)woff2_length, &offset, result);
  Store16((int)tables.size(), &offset, result);
  Store16(0, &offset, result);  // reserved
  // totalSfntSize
  StoreU32((uint32_t)ComputeUncompressedLength(font_collection),
                                               &offset, result);
  ...
  if (compressed_metadata_buf_length > 0) {
    StoreU32((uint32_t)woff2_length - compressed_metadata_buf_length,
             &offset, result);  // metaOffset
    StoreU32(compressed_metadata_buf_length, &offset, result);  // metaLength
    StoreU32((uint32_t)params.extended_metadata.length(),
  ...
  if (font_collection.flavor == kTtcFontFlavor) {
    ...
    Store255UShort((int)font_collection.fonts.size(), &offset, result);
