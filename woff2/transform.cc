...
class GlyfEncoder {
 public:
  ...
  void GetTransformedGlyfBytes(std::vector<uint8_t>* result) {
    ...
    WriteLong(result, (int)n_contour_stream_.size());
    WriteLong(result, (int)n_points_stream_.size());
    WriteLong(result, (int)flag_byte_stream_.size());
    WriteLong(result, (int)glyph_stream_.size());
    WriteLong(result, (int)composite_stream_.size());
    WriteLong(result, (int)(bbox_bitmap_.size() + bbox_stream_.size()));
    WriteLong(result, (int)instruction_stream_.size());
 ...
 private:
  ...
  void WriteSimpleGlyph(int glyph_id, const Glyph& glyph) {
    int num_contours = (int)glyph.contours.size();
    WriteUShort(&n_contour_stream_, num_contours);
    if (ShouldWriteSimpleGlyphBbox(glyph)) {
      WriteBbox(glyph_id, glyph);
    }
    for (int i = 0; i < num_contours; i++) {
      Write255UShort(&n_points_stream_, (int)glyph.contours[i].size());
    }
    int lastX = 0;
    int lastY = 0;
    for (int i = 0; i < num_contours; i++) {
      int num_points = (int)glyph.contours[i].size();
...
bool TransformGlyfAndLocaTables(Font* font) {
  ...
  transformed_glyf->length = (uint32_t)transformed_glyf->buffer.size();
...
bool TransformHmtxTable(Font* font) {
  ...
  transformed_hmtx->length = (uint32_t)transformed_hmtx->buffer.size();
