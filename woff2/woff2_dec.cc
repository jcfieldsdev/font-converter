...
#include "port.hh"
#include "round.hh"
#include "store_bytes.hh"
#include "table_tags.hh"
#include "variable_length.hh"
#include "woff2_common.hh"
...
bool StorePoints(unsigned int n_points, const Point* points,
  ...
  for (unsigned int i = 0; i < n_points; ++i) {
    ...
    if (dx == 0) {
      ...
      x_offset = (uint32_t)Store16(dst, x_offset, dx);
    }
    last_x += dx;
    ...
    if (dy == 0) {
      ...
      y_offset = (uint32_t)Store16(dst, y_offset, dy);
    }
    last_y += dy;
  }
  *glyph_size = y_offset;
...
bool Pad4(WOFF2Out* out) {
  ...
  uint32_t pad_bytes = (uint32_t)(Round4(out->Size()) - out->Size());
  if (pad_bytes > 0) {
...
bool ReconstructGlyf(const uint8_t* data, Table* glyf_table,
  ...
  for (unsigned int i = 0; i < info->num_glyphs; ++i) {
    ...
    loca_values[i] = (uint32_t)(out->Size() - glyf_start);
  ...
  glyf_table->dst_length = (uint32_t)out->Size() - glyf_table->dst_offset;
  loca_table->dst_offset = (uint32_t)out->Size();
  ...
  loca_table->dst_length = (uint32_t)out->Size() - loca_table->dst_offset;
...
size_t StoreTableEntry(uint8_t* result, uint32_t offset, uint32_t tag) {
  offset = (uint32_t)StoreU32(result, offset, tag);
  offset = (uint32_t)StoreU32(result, offset, 0);
  offset = (uint32_t)StoreU32(result, offset, 0);
  offset = (uint32_t)StoreU32(result, offset, 0);
  return offset;
}
...
uint64_t ComputeOffsetToFirstTable(const WOFF2Header& hdr) {
  ...
  if (hdr.header_version) {
    offset = CollectionHeaderSize(hdr.header_version,
      (uint32_t)hdr.ttc_fonts.size()) + kSfntHeaderSize * hdr.ttc_fonts.size();
  ...
  for (size_t i = 0; i < tables.size(); i++) {
    ...
    if (!reused) {
      if ((table.flags & kWoff2FlagsTransform) != kWoff2FlagsTransform) {
        ...
        table.dst_offset = (uint32_t)dest_offset;
...
bool WriteHeaders(const uint8_t* data, size_t length, RebuildMetadata* metadata,
  ...
  if (hdr->header_version) {
    ...
    offset = StoreU32(result, offset, (uint32_t)hdr->ttc_fonts.size());  // ULONG numFonts
    ...
    for (size_t i = 0; i < hdr->ttc_fonts.size(); i++) {
      ...
      offset_table = StoreU32(result, offset_table, (uint32_t)offset);

      // write the actual offset table so our header doesn't lie
      ttc_font.dst_offset = (uint32_t)offset;
      ...
      for (const auto table_index : ttc_font.table_indices) {
        ...
        metadata->font_infos[i].table_entry_by_tag[tag] = (uint32_t)offset;
        offset = StoreTableEntry(result, (uint32_t)offset, tag);
      }
    ...
    for (uint16_t i = 0; i < hdr->num_tables; ++i) {
      metadata->font_infos[0].table_entry_by_tag[sorted_tables[i].tag] = (uint32_t)offset;
      offset = StoreTableEntry(result, (uint32_t)offset, sorted_tables[i].tag);
    }
  }
